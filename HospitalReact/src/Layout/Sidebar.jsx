...
function Sidebar() {
    ...
    return (<div className="bg-white xl:shadow-lg px-4 xl:h-screen w-full border-r border-border overflow-auto">
            <Link to="/">
                <img
                    src="/images/logo.png"
                    alt="logo"
            ...
            `}
                        />
                        <p
                            className={`text-sm font-medium group-hover:text-subMain ${currentPath(item.path) === item.path ? "text-subMain" : "text-gray-500"}`}
                        >
                            {item.title}
                        </p>
                    </Link>))}
